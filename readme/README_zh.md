...
[Elastic License 2.0](../LICENSE)。源码可得（source available），允许自用、修改、再分发；唯一限制是不得将本软件作为托管服务转售。**本项目为 source-available，并非 OSI 定义的 open source。** 详见 [许可证说明](../docs/zh/license.md)。<!-- banned-word-allow -->
