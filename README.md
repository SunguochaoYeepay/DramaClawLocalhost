...
DramaClaw 的代码还没有放出，不是因为我们想做一个只展示概念的项目。
...
现在正在做的准备工作包括代码清理、配置脱敏、文档补全、示例项目整理、部署流程验证，以及许可证和贡献流程的最后确认。我们希望第一次放出的就是可以被认真阅读、部署、复用和二次开发的版本，而不是一个临时快照。

代码在路上。感谢理解，也欢迎先 star 关注发布进度。
